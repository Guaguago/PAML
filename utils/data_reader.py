...
# Prepare examples for CVAE-dialog
class Dataset_CVAE(Dataset):
    def __getitem__(self, index):
        """Returns one data pair (source and target)."""
        item = {}
        item["input_txt"] = self.src[index]
    ...
    def preprocess(self, arr, anw=False):
        ...
        if (anw):
            sequence = [config.SOS_idx] + [
                self.vocab.word2index[word] if word in self.vocab.word2index else config.UNK_idx for word in
                arr.split(' ')] + [config.EOS_idx]
        else:
            sequence = [config.SOS_idx] + [
                self.vocab.word2index[word] if word in self.vocab.word2index else config.UNK_idx for word in
                ' '.join(arr).split(' ')] + [config.EOS_idx]
        return torch.LongTensor(sequence)
...
def collate_CVAE_fn(data):
    ...
    # add for CVAE model
    len_labels = target_lengths - 1
    masks = (1 - F.one_hot(len_labels, len_labels.max() + 1).cumsum(1))[:, :-1]  # [batch, len_decoder]
    batch_size = masks.size(0)
    sampled_latents = torch.randn((batch_size, Model_Config.latent_size))  # [batch, latent_size]
    ...
    if config.USE_CUDA:
        ...
        input_lengths = input_lengths.cuda()
        target_lengths = target_lengths.cuda()
        sampled_latents = sampled_latents.cuda()
        masks = masks.cuda()

    d = {}
    d['posts'] = input_batch
    d['responses'] = target_batch
    d['len_posts'] = input_lengths
    d['len_responses'] = target_lengths
    d['sampled_latents'] = sampled_latents  # [batch, latent_size]
    d['masks'] = masks.float()
...
def prepare_data_seq():
    ...
    train = preprocess(train,
                       vocab)  # {persona:{dial1:[[context,canditate,answer,persona],[context,canditate,answer,persona]]}, dial2:[[context,canditate,answer,persona],[context,canditate,answer,persona]]}}
...
class Personas:
    ...
    def get_num_of_dialog(self, persona, split):
        return len(self.type[split][persona])
...
class Personas_CVAE(Personas):
    def get_balanced_loader(self, persona, batch_size, split, fold=-1, dial_num=1):
        ...
        dataset_train = Dataset_CVAE(tr, self.vocab)
        data_loader_tr = torch.utils.data.DataLoader(dataset=dataset_train,
                                                     batch_size=batch_size,
                                                     shuffle=True,
                                                     collate_fn=collate_CVAE_fn)

        dataset_valid = Dataset_CVAE(val, self.vocab)
        data_loader_val = torch.utils.data.DataLoader(dataset=dataset_valid,
                                                      batch_size=batch_size,
                                                      shuffle=True,
                                                      collate_fn=collate_CVAE_fn)
